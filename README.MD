...
# Create environment
conda create -n rc-track-analyzer python=3.10 -y
conda activate rc-track-analyzer

# Install PyTorch with CUDA (for GPU support)
pip install torch torchvision --index-url https://download.pytorch.org/whl/cu121

# Or for CPU-only (slower but works without NVIDIA GPU)
# pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu

# Install other dependencies
pip install -r requirements.txt

# Install SAM2 (must be done separately - see note below)
...
> **Note on SAM2 installation:** The SAM2 package must be installed directly from GitHub using `pip install git+https://github.com/facebookresearch/sam2.git`. The syntax `sam2 @ git+...` in requirements.txt does not work because the package metadata uses the name `sam-2` instead of `sam2`.
...
![Homography transformation example](docs/homography_transformation_example.png)

The homography transforms the side-view camera footage into a birds-eye-view (BEV), correcting for perspective distortion. The grid overlay (1 m spacing) confirms that the real-world scale is preserved — objects and distances in the BEV match their actual physical measurements.

### Trajectory & metrics

![Metrics example](docs/metric_example.png)

After tracking and homography, the system produces:
- **BEV trajectory overlay** — the car's path color-coded by acceleration (green), braking (red), and constant speed (yellow), with the start/finish line marked as S/F
- **Speed profile** — speed in km/h over time
- **G-Force profile** — longitudinal and lateral G-forces over time
